...
function getMapsScriptsWithAliases(maps) {
    const scripts = {};
    const scriptPathToAlias = new Map();
...
export function getMapsOptimizers(maps, options) {
    ...
    for (const [mapPath, map] of maps) {
        ...
        optionsParsed.output.tileset.suffix = crypto
            ...
            .digest("hex");
        plugins.push(mapOptimizer(mapPath, map, distFolder, structuredClone(optionsParsed), baseDistPath, playUrl, scriptPathToAlias));
    }
    return plugins;
}
// Map Optimizer Vite Plugin
function mapOptimizer(mapPath, map, distFolder, optimizeOptions, baseDistPath, playUrl, scriptPathToAlias) {
    let resolvedOutDir;
    ...
    return {
        ...
        async writeBundle() {
            ...
            const optimizedMapObject = JSON.parse(optimizedMapFile.toString());
